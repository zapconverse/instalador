...
import SendFlowManuallyService from "../services/FlowBuilderService/SendFlowManuallyService";
...
export const SendFlowManually = async (req: Request, res: Response) => {
  const { ticketId, flowId } = req.body;
  const { companyId } = req.user;

  try {
    await SendFlowManuallyService({
      ticketId,
      flowId,
      companyId
    });

    return res.status(200).json({ message: "Flow enviado com sucesso" });
  } catch (err) {
    console.error("Error sending flow manually:", err);
